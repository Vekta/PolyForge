...
import { recordUsage, detectCostRegression } from './telemetry.js';
import { evaluateWorkPredicate } from './work-predicate.js';
...
export async function runRoutine({ projectRoot, routineName, runNow = false, dry = false }) {
  ...
  let wt;
  try {
    if (routine.work_predicate) {
      const { hasWork, reason } = evaluateWorkPredicate(routine.work_predicate, { projectRoot });
      if (!hasWork) {
        logEvent(routineName, { type: 'skip', reason: 'no-work', predicate: routine.work_predicate.type, detail: reason });
        return { ok: true, skipped: 'no-work', reason };
      }
    }
    ...
    if (typeof routine.max_thinking_tokens === 'number') {
      env.MAX_THINKING_TOKENS = String(routine.max_thinking_tokens);
    }
    ...
    if (parsed) {
      ...
      const regression = detectCostRegression(routineName, costUsd);
      recordUsage({
        routine: routineName,
      ...
      if (regression.regressed) {
        logEvent(routineName, {
          type: 'cost-regression',
          costUsd,
          avgCostUsd: regression.avgCostUsd,
