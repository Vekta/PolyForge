...
export async function runRoutine({ projectRoot, routineName, runNow = false, dry = false }) {
  ...
  try {
    ...
    return { ok: !parsed?.is_error, parsed, worktree: wt, dry: effectiveDry };
  } catch (e) {
    logEvent(routineName, { type: 'error', msg: e.message });
    return { ok: false, error: e.message };
  } finally {
    if (config.concurrency?.serial_lock) releaseLock();
  }
}
...
function loadPrompt(projectRoot, routine) {
  if (!routine.template?.startsWith('builtin:')) {
    throw new Error(`Non-builtin templates not supported yet: ${routine.template}`);
  }
  const name = routine.template.replace('builtin:', '');
